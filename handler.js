...
this.reply(m.chat, `🇦🇱 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝙔𝘼𝙇𝙇𝙄𝘾𝙊𝘾𝙊𝙄𝙉𝙎`, m)
continue     
}
...
if (m.money)
m.reply(+m.money + ' 𝙔𝘼𝙇𝙇𝙄𝘾𝙊𝘾𝙊𝙄𝙉𝙎 🇦🇱 𝙐𝙎𝘼𝘿𝙊(𝙎)')  
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '🧡 𝗕𝗼𝘁 𝗗𝗲 𝗠𝗮𝗴𝘆 💛') :
...
"thumbnail": apii.data, 
"title": [wm, '🧡 𝗕𝗼𝘁 𝗗𝗲 𝗠𝗮𝗴𝘆 💛'].getRandom(),
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '🧡 𝗕𝗼𝘁 𝗗𝗲 𝗠𝗮𝗴𝘆 💛'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, nna, nn, nnn, yt, ig, paypal, fb].getRandom() }}}}, tg)
