...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  const sessionPath = './YallicoSex/';
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '*[❗] La carpeta YallicoSex no existe o está vacía.*'}, {quoted: m});
    }
    ...
    if (filesDeleted === 0) {
      await conn.sendMessage(m.chat, {text: '*[❗] No se encontró ningún archivo para eliminar en la carpeta YallicoSex.*'}, {quoted: m});
    } else {
