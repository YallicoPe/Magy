...
mobile: MethodMobile, 
browser: opcion == '1' ? ['YallicoBot-MD', 'Edge', '2.0.0'] : methodCodeQR ? ['YallicoBot-MD', 'Edge', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
console.log(chalk.bgBlack(chalk.bold.redBright(`CONFIGURAR ARCHIVO ${chalk.bold.greenBright("config.js")} SU NÚMERO DE WHATSAPP NO TIENE CÓDIGO DE PAÍS, ${chalk.bold.yellowBright("EJEMPLO: +51926933818")}`)))
process.exit(0)
}} else {
while (true) {
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "YallicoBotSex"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./YallicoSex/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./YallicoSex")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./YallicoSex/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./YallicoBotSex/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./YallicoBotSex/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./YallicoBotSex/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./YallicoBotSex/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./YallicoSex/', './YallicoBotSex/']
